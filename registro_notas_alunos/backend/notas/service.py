...
class NotasService:
    ...
    def criar(self, notas: Notas) -> int:
        ...
        if self.buscar_por_matricula(notas.id_matricula):
            raise Exception("Já existem notas registradas para esta matrícula")
        ...
        query = """
            ...
            RETURNING id
        """
        params = (notas.id_matricula, notas.sm1, notas.sm2, notas.av, 
                 notas.avs, notas.nf, notas.situacao)
    ...
    def atualizar(self, notas: Notas) -> None:
        ...
        if not self.buscar_por_id(notas.id):
            raise Exception(f"Registro de notas com ID {notas.id} não encontrado")
        ...
        query = """
            ...
            WHERE id = %s
        """
        params = (notas.sm1, notas.sm2, notas.av, notas.avs, 
                 notas.nf, notas.situacao, notas.id)
    ...
    def listar_por_disciplina(self, id_disciplina: int) -> List[Tuple[str, str, float, float, float, float, float, str]]:
        """
    ...
    def listar_por_aluno(self, id_aluno: int) -> List[Tuple[str, int, int, float, float, float, float, float, str]]:
        """
    ...
    def excluir(self, id: int) -> None:
        ...
        if not self.buscar_por_id(id):
            raise Exception(f"Registro de notas com ID {id} não encontrado")
    ...
    def excluir_por_matricula(self, id_matricula: int) -> None:
        ...
        if not notas:
            raise Exception("Notas não encontradas para esta matrícula")
